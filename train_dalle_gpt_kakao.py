...
if not ENABLE_WEBDATASET:
    ...
    ).exists(), f"The path {args.image_text_folder} was not found."
else:
...
def tokenize(s):
    return tokenizer.tokenize(
        s.decode("utf-8"), TEXT_SEQ_LEN, truncate_text=args.truncate_captions
    ).squeeze(0)
...
if ENABLE_WEBDATASET:
    ...
    ds = (
        ...
        .batched(BATCH_SIZE / distr_backend.get_world_size(), partial=True)
    )
else:
    ds = TextImageDataset(
        args.image_text_folder,
...
if ENABLE_WEBDATASET:
    ...
    dl = wds.WebLoader(
        ds, batch_size=None, shuffle=False, num_workers=4
...
dalle = DALLE_gpt_kakao(vae=vae, **dalle_params)
...
(distr_dalle, distr_opt, distr_dl, distr_scheduler) = distr_backend.distribute(
    ...
    training_data=((None if ENABLE_WEBDATASET else ds) if using_deepspeed else dl),
...
def save_model(path, epoch=0):
    ...
    if using_deepspeed:
        ...
        if (
            deepspeed_config.get("zero_optimization", {}).get("stage", 0) >= 2
